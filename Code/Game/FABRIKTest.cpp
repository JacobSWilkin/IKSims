...
void FABRIKTest::Update()
{
	...
	{
		m_targetPosition = Vec3(0.0f, sinf(static_cast<float>(totalTime) * 2.0f) * 5.f, 0.5f);
	}

	std::vector<int> boneChain;
	for (int boneIndex = 0; boneIndex < static_cast<int>(m_skeleton.m_bones.size()); ++boneIndex) 
	{
		boneChain.push_back(boneIndex);
	}
	m_skeleton.SolveFABRIK(boneChain, m_targetPosition);
...
void FABRIKTest::TargetPositionMovement(float deltaSeconds)
{
	if (g_theInput->IsKeyDown('I'))
	{
		m_targetPosition.x += TARGET_MOVE_SPEED * static_cast<float>(deltaSeconds);
	}
	...
	{
		m_targetPosition.y += TARGET_MOVE_SPEED * static_cast<float>(deltaSeconds);
	}
	...
	{
		m_targetPosition.z += TARGET_MOVE_SPEED * static_cast<float>(deltaSeconds);
	}
...
void FABRIKTest::RenderGrid() const
{
	g_theRenderer->SetModelConstants();
	SetWorldConstants();
...
void FABRIKTest::SetWorldConstants() const
{
