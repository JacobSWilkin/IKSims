...
float4 PixelMain(v2p_t input) : SV_Target0
{
	...
	//----------------------------------------ALPHA FADE-----------------------------------------------------//
	float distanceToCamera = length(CameraPosition - input.worldPosition.xyz);
	float fadeStart = FogStart;
	float fadeEnd   = FogEnd;

	float fade = saturate((distanceToCamera - fadeStart) / (fadeEnd - fadeStart));
	fade = SmoothStep3(fade);

	// Inverting the alpha fade
